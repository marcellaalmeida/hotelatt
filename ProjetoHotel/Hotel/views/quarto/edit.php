<?php
    // Buscar as informações do quarto para atualizar
...
<head>
    ...
    <title>Sistema Hotel - Editar Quarto</title>
...
<body> <svg xmlns="http://www.w3.org/2000/svg" class="d-none">
    ...
    <header class="navbar sticky-top bg-dark flex-md-nowrap p-0 shadow" data-bs-theme="dark"> <a
            class="navbar-brand col-md-3 col-lg-2 me-0 px-3 fs-6 text-white" href="#">Sistema Hotel</a>
                ...
                <div class="my-4">
                    ...
                    <form action="update.php" method="post">
                        <div class="form-group">
                            ...
                            <input type="number" name="numero" id="numero" class="form-control" value="<?= $quarto->getNumero() ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="tipo" class="form-label">Tipo de Quarto</label>
                            <select name="tipo" id="tipo" class="form-control" required>
                                <option value="">Selecione o tipo</option>
                                <option value="Solteiro" <?= $quarto->getTipo() == 'Solteiro' ? 'selected' : '' ?>>Solteiro</option>
                                <option value="Casal" <?= $quarto->getTipo() == 'Casal' ? 'selected' : '' ?>>Casal</option>
                                <option value="Luxo" <?= $quarto->getTipo() == 'Luxo' ? 'selected' : '' ?>>Luxo</option>
                                <option value="Suíte" <?= $quarto->getTipo() == 'Suíte' ? 'selected' : '' ?>>Suíte</option>
                                <option value="Presidencial" <?= $quarto->getTipo() == 'Presidencial' ? 'selected' : '' ?>>Presidencial</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="preco" class="form-label">Preço por Noite (R$)</label>
                            <input type="number" name="preco" id="preco" class="form-control" step="0.01" min="0" value="<?= $quarto->getPreco() ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="status" class="form-label">Status</label>
                            <select name="status" id="status" class="form-control" required>
                                <option value="">Selecione o status</option>
                                <option value="Disponível" <?= $quarto->getStatus() == 'Disponível' ? 'selected' : '' ?>>Disponível</option>
                                <option value="Ocupado" <?= $quarto->getStatus() == 'Ocupado' ? 'selected' : '' ?>>Ocupado</option>
                                <option value="Manutenção" <?= $quarto->getStatus() == 'Manutenção' ? 'selected' : '' ?>>Manutenção</option>
                                <option value="Reservado" <?= $quarto->getStatus() == 'Reservado' ? 'selected' : '' ?>>Reservado</option>
                            </select>
                        </div>
                        <input type="hidden" name="id" value="<?= $quarto->getIdquarto() ?>">
                        <div class="form-group mt-4">
                            <input type="reset" value="Limpar" class="btn btn-secondary">
                            <input type="submit" value="Atualizar Quarto" class="btn btn-primary">
                        </div>
                    </form>
                </div>
            </main>
